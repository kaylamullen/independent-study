...
class System:
    def __init__(self, graph):
        ...
        self.dist_matrix = bellman_ford(self.matrix_graph, directed=True, indices=range(109))
        self.next_node = {}
        for i in range(109):
            self.next_node[i] = {}
            for j in range(109):
...
class Baseline(Simulator):
    ...
    ####### POTENTIAL SOLUTION TO SYSTEM PROBLEM BELOW #######
    # def __init__(self, start_time: float = 0.0, system : System = System(WeightedGraph())) -> None:
    ...
    def handle(self, event_id: str, payload: Any) -> None:
        if event_id == 'Task Arrival':
            task = payload[0]
            system = payload[1]
            system.task_list.append(task)
            system.total_possible_reward = system.total_possible_reward+task.reward
            # self.schedule_at(task.target_time, 'Task Expiry', (tassk, system))
            
        # elif event_id == 'Task Expiry':
        #     task = payload[0]
        #     system = payload[1]
        #     system.remove_task(task)
            ...
            dasher = payload[0]
            system = payload[1]
            # print(f"dasher exit time: {dasher.exit_time}")
            # print(f"now: {self.now}")
            system.dasher_list.append(dasher)
            graph = system.graph
            if dasher.exit_time <= self.now:
                system.remove_dasher(dasher)
            
            
            # make sure there are no tasks that have expired in the system
            for task in system.task_list:
                if task.target_time < self.now:
                    system.remove_task(task)
                    
            available_tasks = system.task_list
            
            if len(available_tasks) > 0:
                # print(f"")
                # intialize the closest task so far as the first task in available tasks
                # available_task = available_tasks[0]
                # while available_tasks.target_time < self.now:
                #     available_tasks.pop()
                    
                closest_so_far = system.dist_matrix[dasher.location][available_tasks[0].location]
                closest_task = available_tasks[0]
                
                # loop through the rest of the tasks currently available to find the closest task
                for available_task in available_tasks[1:]:
                    time = system.dist_matrix[dasher.location][task.location]
                    # if we find an expired task --> mark it as unavailable and don't choose
                    if available_task.target_time < self.now:
                        system.remove_task(available_task)
                        continue
                    
                    # if the new time found is better than the closest so far, update this task as the closest so far
                    elif time < closest_so_far:
                        closest_so_far, closest_task = time, available_task

                # closest task has been found now -->
                # print(f'closest task: {closest_so_far}')
                # make sure closest task can be completed before dasher exit time or task has already expired
                if closest_task.target_time > dasher.exit_time or closest_task.target_time < self.now: # QUESTION: if a dasher finishes their task at the same time as when they exit, can they still complete it?
                    # if the dasher cannot finish the task in time --> check back later for another task
                    self.schedule_at(self.now + 1, 'Dasher Arrival', (dasher, system))
                else:
                    # the task is feasible for the dasher to complete -->
                    
                    # update dasher location to task location
                    dasher.location = closest_task.location
                    # make new event for dasher arrival at the task's target time
                    if closest_task.target_time < dasher.exit_time:
                        system.remove_task(closest_task)
                        self.schedule_at(closest_task.target_time, 'Dasher Arrival', (dasher, system))
                        #we will only process reward if the system gets to that tinme (this accounts for the case where our system ends before task target_time)
                        self.schedule_at(closest_task.target_time, 'Collect Reward', (closest_task, system))
                        # remove closest task from system, making it unavailable
                        
                    
                        ####### PROBLEM #######
                            # If we are passing in system with the payload, then at the time the dasher 
                            # arrives again, they're going to have an outdated list of available tasks 
                            # bc another dasher could have picked one of them up by the time they're done 
                            # completing the current task
                        ####### POTENTIAL SOLUTION #######
                            # make the system an attribute of the simulator so every dasher can access updates to the system
                            # see commented implementation in Baseline __init__()
                
            elif self.now + 1 < dasher.exit_time:
                # if there are no available tasks --> schedule new event for dasher to check for new task at t+1
                self.schedule_at(self.now + 1, 'Dasher Arrival', (dasher, system))
                
            #run dijkstra
            #find closest task r that is feasable
                #if it exists create new dasher arrival event (at location of the task r, at time (time+ time it takes them to get to r))
                #remove r from task list
                #add reward to system 
        elif event_id == 'Collect Reward':
            system = payload[1]
            system.add_reward(payload[0])
...
#     # converting string date-time to pandas timestamp
#     task_df['TIME_NUMERIC'] = pd.to_datetime(task_df['TIME'], format='%a %b %d %H:%M:%S %z %Y')
    
#     # converting date time to floats starting at 0 for earliest task --> usable by simulator
#     min_time = min(task_df['TIME_NUMERIC'])
#     timestamps = task_df['TIME_NUMERIC']
#     times_since_start = timestamps - min_time
#     time_as_float_secs = (times_since_start.dt.total_seconds()) / 60 # tasks separated by number of mins btwn each real task
#     task_df['TIME_AS_FLOAT'] = time_as_float_secs
#     tasks = []
...
#         target_time = row['TIME_AS_FLOAT'] + time_avail
#         # target_time = row['TIME_AS_FLOAT'] + row['TIME_AVAIL']
#         reward = random.randint(1, 5)
#         task = Task(location=row['VERTEX'], appear_time=row['TIME_AS_FLOAT'], target_time=target_time, reward=reward)
#         tasks.append(task)
    return tasks
...
def schedule_events(sim, dashers, tasks, system, stop_time):
        for task in tasks:
            sim.schedule_at(task.appear_time, 'Task Arrival', (task, system))
        for dasher in dashers:
            sim.schedule_at(dasher.start_time, 'Dasher Arrival', (dasher, system))
...
def test_diff_dasher_amts(dasher_fname, task_fname, stop_time):
    ...
    for num_tasks in [100, 200, 300, 400, 500, 699]:
        ...
        i = 0
    with open('baseline_results.csv', mode='w', newline='') as file:
...
if __name__ == "__main__":
    ...
    # schedule_events(sim, dashers_small, tasks_small, system, 1500)
    # sim.run()
    # print("events processed:", sim.events_processed)
    # print(f"total reward: {system.total_reward}")
    # print(f"max possible reward: {system.total_possible_reward}")
